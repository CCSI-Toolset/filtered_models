...
! Sarkar sub-grid models - 07/2013
      DOUBLE PRECISION dragForceCylSuspX
      DOUBLE PRECISION solidsRatio
      DOUBLE PRECISION betaStarCylSuspX, betaStarCylSuspY, uSolidStar
...
! The cylinder-suspensiion drag imposed on the solids phase, 
! not partitioned betn gas & solids. Modeled as a source term.  
                  IF (SG_CYL_HYDRO) THEN
                     dragForceCylSuspX = ZERO
                     IF (M == 1 .AND. EP_S(IJK,2) > ZERO .AND. FLUID_AT(IJK)) THEN !drag exerted only on the flowing solids phase
                        ...
                        !this calculation is being repeated, any way to avoid/optmize?
                        betaStarCylSuspY = (B1*solidsRatio**2) &
                             / (ONE+B2*solidsRatio**2)

                        betaStarCylSuspX = betaStarCylSuspY & 
                             *B3/(ONE + B4*(solidsRatio**2)) &
                             *(ONE + 16.02*(solidsRatio**2))/1.042
                        
                        !imjk = IM_OF(ijk) ! calculated earlier
                        uSolidStar = HALF*(U_s(ijk,M) + U_s(imjk,M))/V_t
                        !dimensionless (avge) cell center solids X-velocity 
                        
                        !drag force due to cylinders on solids, per unit volume
                        dragForceCylSuspX = RO_S(IJK,1) * GRAVITY * (ONE - EP_S(IJK,2)) & 
                             *(-betaStarCylSuspX*uSolidStar*abs(uSolidStar) )

                     ENDIF !(M /= cylPhaseIndex .AND. FLUID_AT(IJK))
                  ENDIF !(SUBGRID_GPC2D_DRAG)
...
! Sarkar sub-grid models - 07/2015
! Add the contridution due to the subgrid cylinder-solids drag - Sarkar
                  IF (SG_CYL_HYDRO) THEN
                     B_M(IJK,M) = B_M(IJK,M) - dragForceCylSuspX*VOL_U(IJK)
                  ENDIF
